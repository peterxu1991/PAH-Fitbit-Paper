...
%% Organize data for PAH patients
...
%% Find # of minutes of sustained walking for PAH patients
...
%% SC Histogram: "Intensity" Parameter for PAH patients (exponential fit)
...
%     TextLocation(['Median = ' num2str(sprintf('%.3g',med)), ', SD = ' num2str(sprintf('%.3g',sig)),...
%         newline, 'Skewness = ' num2str(sprintf('%.3g',sk)), ', KS-statistics = ' num2str(sprintf('%.3g',ks)),...
%         newline, newline, 'Avg 1/e value = ' num2str(sprintf('%.3g',this_weekly_raw(i).avg_int_expdist_mu))],...
%         'Location','northeast');
...
%% SC Histogram: "Intensity" Parameter for PAH patients (lognormal fit)
...
%% Random: Finding maximum sustained duration value
...
for i = 1:length(this_weekly_raw)
    temp1 = [];
    for j = 1:length(this_weekly_raw(i).sustained)
        temp1 = [temp1; this_weekly_raw(i).sustained(j).final_duration];
    end
    this_weekly_raw(i).max_duration = max(temp1);
end
...
fieldnames = {'Name','avg_end_expdist_mu','avg_int_expdist_mu','avg_freq','product'};
...
%% Graph: endurance/frequency vs intensity/frequency
...
title('Endurance/freq vs Intensity/freq for PAH Patients')
...
b = num2str(PAH_indices'); c = cellstr(b);
...
y = @(x) (x*p1(1) + p1(2));
fplot(y,'-k','LineWidth',1)
title('log(endurance/freq) vs log(intensity/freq) for PAH Patients')
...
[p2,S2] = polyfit(intensity, endurance, 1);
y = @(x) (x*p2(1) + p2(2));
fplot(y,'-k','LineWidth',1)
title('Endurance vs Intensity for PAH Patients')
xlabel('Intensity')
ylabel('Endurance')
TextLocation(['y = ' num2str(sprintf('%.3g',p2(1))), '*x + ' num2str(sprintf('%.3g',p2(2))),...
    newline,newline,'Norm of residuals = ' num2str(sprintf('%.3g',S2.normr))],...
    'Location','northwest');
...
%% Export data for PCA (optional)

this_weekly_raw = weekly_raw_PAH;

PAH_pca_struct = struct([]);

for i = 1:length(this_weekly_raw)
    temp1 = zeros(length(this_weekly_raw(i).sustained),1); 
    temp2 = zeros(length(this_weekly_raw(i).sustained),1);
    temp3 = zeros(length(this_weekly_raw(i).sustained),1);
    for j = 1:length(this_weekly_raw(i).sustained)
        if ~isempty(this_weekly_raw(i).sustained(j).expdist_mu)
            temp1(j) = this_weekly_raw(i).sustained(j).expdist_mu;
        end
        
        if ~isempty(this_weekly_raw(i).sustained(j).compileddata)
            temp2(j) = size(this_weekly_raw(i).sustained(j).compileddata,1);
        end
        
        temp3(j) = this_weekly_raw(i).SChist(j).expdist_mu;
    end
    
    PAH_pca_struct(i).name = this_weekly_raw(i).name;
    PAH_pca_struct(i).end_expdist_mu = temp1;
    PAH_pca_struct(i).freq = temp2;
    PAH_pca_struct(i).int_expdist_mu = temp3;
end

% Save PAH_pca_struct as a .mat file!
