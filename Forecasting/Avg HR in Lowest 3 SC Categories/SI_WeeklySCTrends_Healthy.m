...
%% Healthy Subjects: calculate avg HR for SC categories
...
%% Slope / intercept using polyfit for Healthy Subjects
...
%% Plot LSF lines for each individual for Healthy Subjects

figure(1)
hold on
for i = 1:length(healthy_indices)
    y = @(x) (weekly_raw_healthy(i).meanslope*x + weekly_raw_healthy(i).meanint);
    fplot(y,[0,150], 'LineWidth',2)
end
title('LSF Lines for Healthy Subjects (average weekly LSF values)')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f) (Incheol)',weekly_raw_healthy(1).meanslope,...
    weekly_raw_healthy(1).meanint);
s2 = sprintf('y = (%.2f) x + (%.2f) (Nicole)',weekly_raw_healthy(2).meanslope,...
    weekly_raw_healthy(2).meanint);
s3 = sprintf('y = (%.2f) x + (%.2f) (PeterX)',weekly_raw_healthy(3).meanslope,...
    weekly_raw_healthy(3).meanint);
s4 = sprintf('y = (%.2f) x + (%.2f) (PeterS)',weekly_raw_healthy(4).meanslope,...
    weekly_raw_healthy(4).meanint);
s5 = sprintf('y = (%.2f) x + (%.2f) (Seyvonne)',weekly_raw_healthy(5).meanslope,...
    weekly_raw_healthy(5).meanint);
...
hold on
for i = 1:length(healthy_indices)
    y1 = @(x) (weekly_raw_healthy(i).meanslope1*x + weekly_raw_healthy(i).meanint1);
    fplot(y1,[0,150], 'LineWidth',2)
end
title('LSF Lines for Healthy Subjects (average weekly LSF values): Region 1')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f) (Incheol)',weekly_raw_healthy(1).meanslope1,...
    weekly_raw_healthy(1).meanint1);
s2 = sprintf('y = (%.2f) x + (%.2f) (Nicole)',weekly_raw_healthy(2).meanslope1,...
    weekly_raw_healthy(2).meanint1);
s3 = sprintf('y = (%.2f) x + (%.2f) (PeterX)',weekly_raw_healthy(3).meanslope1,...
    weekly_raw_healthy(3).meanint1);
s4 = sprintf('y = (%.2f) x + (%.2f) (PeterS)',weekly_raw_healthy(4).meanslope1,...
    weekly_raw_healthy(4).meanint1);
s5 = sprintf('y = (%.2f) x + (%.2f) (Seyvonne)',weekly_raw_healthy(5).meanslope1,...
    weekly_raw_healthy(5).meanint1);
TextLocation([s1,newline,s2,newline,s3,newline,s4,newline,s5],'Location','northwest')
legend('Incheol','Nicole','PeterX','PeterS','Seyvonne','Location','best')
hold off
...
hold on
for i = 1:length(healthy_indices)
    y2 = @(x) (weekly_raw_healthy(i).meanslope2*x + weekly_raw_healthy(i).meanint2);
    fplot(y2,[0,150], 'LineWidth',2)
end
title('LSF Lines for Healthy Subjects (average weekly LSF values): Region 2')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f) (Incheol)',weekly_raw_healthy(1).meanslope2,...
    weekly_raw_healthy(1).meanint2);
s2 = sprintf('y = (%.2f) x + (%.2f) (Nicole)',weekly_raw_healthy(2).meanslope2,...
    weekly_raw_healthy(2).meanint2);
s3 = sprintf('y = (%.2f) x + (%.2f) (PeterX)',weekly_raw_healthy(3).meanslope2,...
    weekly_raw_healthy(3).meanint2);
s4 = sprintf('y = (%.2f) x + (%.2f) (PeterS)',weekly_raw_healthy(4).meanslope2,...
    weekly_raw_healthy(4).meanint2);
s5 = sprintf('y = (%.2f) x + (%.2f) (Seyvonne)',weekly_raw_healthy(5).meanslope2,...
    weekly_raw_healthy(5).meanint2);
TextLocation([s1,newline,s2,newline,s3,newline,s4,newline,s5],'Location','northwest')
legend('Incheol','Nicole','PeterX','PeterS','Seyvonne','Location','best')
hold off


%% Plot average HR in different SC bins (weekly) for Healthy Subjects
...
for i = 1:length(weekly_raw_healthy)
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata1 = [xdata1; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata1 = [ydata1; weekly_raw_healthy(i).SCbins(j).meanhr_lowsteps];  
    end
    figure(i)
    hold on
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata2 = [xdata2; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata2 = [ydata2; weekly_raw_healthy(i).SCbins(j).meanhr_medsteps];  
    end
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata3 = [xdata3; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata3 = [ydata3; weekly_raw_healthy(i).SCbins(j).meanhr_pursteps];  
    end
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata4 = [xdata4; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata4 = [ydata4; weekly_raw_healthy(i).SCbins(j).meanhr_slowwalk];  
    end
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata5 = [xdata5; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata5 = [ydata5; weekly_raw_healthy(i).SCbins(j).meanhr_medwalk];  
    end
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata6 = [xdata6; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata6 = [ydata6; weekly_raw_healthy(i).SCbins(j).meanhr_briskwalk];  
    end
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata7 = [xdata7; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata7 = [ydata7; weekly_raw_healthy(i).SCbins(j).meanhr_fastamb];  
    end
    ...
    for j = 1:length(weekly_raw_healthy(i).SCbins)
        xdata8 = [xdata8; strcat('Week', string(weekly_raw_healthy(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_healthy(i).SCbins(j).Year))];
        ydata8 = [ydata8; weekly_raw_healthy(i).SCbins(j).meanhr_run];  
    end
    ...
    title(sprintf('Mean HR for Healthy Subject %s',string(healthy_indices(i))))
    xlabel('Time')
    ylabel('Mean HR (BPM)')
    hold off
end
...
for i = 1:length(this_weekly_raw)
    ...
    legend('Low Steps', 'Medium Steps', 'Purposeful Steps', 'Slow Walk', 'Medium Walk', ...
        'Brisk Walk', 'Fast Ambulation', 'Run','Location','eastoutside')
    title(sprintf('Minutes in Each SC Category for Healthy Subject %s',string(healthy_indices(i))))
    xlabel('Week')
...
% % Amount of time (absolute) spent in each SC category (Method 2 - line graph)
% for i = 1:length(this_weekly_raw)
%     xdata1 = []; ydata1 = [];
%     xdata2 = []; ydata2 = [];
%     xdata3 = []; ydata3 = [];
...
%     
%     for j = 1:length(this_weekly_raw(i).SCbins)
%         xdata4 = [xdata4; strcat('Week', string(this_weekly_raw(i).SCbins(j).Week),...
%             ' Year', string(this_weekly_raw(i).SCbins(j).Year))];
%         ydata4 = [ydata4; length(this_weekly_raw(i).SCbins(j).sc_slowwalk)];  
%     end
%     xdata4 = reordercats(categorical(xdata4),xdata4);
...
%     
%     for j = 1:length(this_weekly_raw(i).SCbins)
%         xdata5 = [xdata5; strcat('Week', string(this_weekly_raw(i).SCbins(j).Week),...
%             ' Year', string(this_weekly_raw(i).SCbins(j).Year))];
%         ydata5 = [ydata5; length(this_weekly_raw(i).SCbins(j).sc_medwalk)];  
%     end
%     xdata5 = reordercats(categorical(xdata5),xdata5);
...
%     
%     for j = 1:length(this_weekly_raw(i).SCbins)
%         xdata6 = [xdata6; strcat('Week', string(this_weekly_raw(i).SCbins(j).Week),...
%             ' Year', string(this_weekly_raw(i).SCbins(j).Year))];
%         ydata6 = [ydata6; length(this_weekly_raw(i).SCbins(j).sc_briskwalk)];  
%     end
%     xdata6 = reordercats(categorical(xdata6),xdata6);
...
%     if isfield(this_weekly_raw(i).SCbins, 'sc_fastamb')
%         for j = 1:length(this_weekly_raw(i).SCbins)
%             xdata7 = [xdata7; strcat('Week', string(this_weekly_raw(i).SCbins(j).Week),...
%                 ' Year', string(this_weekly_raw(i).SCbins(j).Year))];
%             ydata7 = [ydata7; length(this_weekly_raw(i).SCbins(j).sc_fastamb)];  
%         end
%         xdata7 = reordercats(categorical(xdata7),xdata7);
...
%     if isfield(this_weekly_raw(i).SCbins, 'sc_run')
%         for j = 1:length(this_weekly_raw(i).SCbins)
%             xdata8 = [xdata8; strcat('Week', string(this_weekly_raw(i).SCbins(j).Week),...
%                 ' Year', string(this_weekly_raw(i).SCbins(j).Year))];
%             ydata8 = [ydata8; length(this_weekly_raw(i).SCbins(j).sc_run)];  
%         end
%         xdata8 = reordercats(categorical(xdata8),xdata8);
...
%     legend('Low Steps', 'Medium Steps', 'Purposeful Steps', 'Slow Walk', 'Medium Walk', ...
%         'Brisk Walk', 'Fast Ambulation', 'Run','Location','eastoutside')
%     title(sprintf('Percentage of Minutes in Each SC Category for PC Patient %s',string(healthy_indices(i))))
%     xlabel('Time')
%     ylabel('% of minutes in each SC category')
%     hold off
% end
...
%% Healthy Forecasting: calculate mean, SD, and LSF line
...
for i = 1:length(this_weekly_raw)
    if length(this_weekly_raw(i).weekly_raw) < weekthresh
        ...
        numentries = length(daily_Healthy(healthy_indices(i)).hr_mu0) - weekthresh;
        
        % Store mean HR and SD for lowsteps, medsteps, and pursteps.
        meanhr_lowsteps = []; meanhr_medsteps = []; meanhr_pursteps = [];
        for j = 1:length(weekly_raw_healthy(i).weekly_raw)
            meanhr_lowsteps = [meanhr_lowsteps weekly_raw_healthy(i).SCbins(j).meanhr_lowsteps];
            meanhr_medsteps = [meanhr_medsteps weekly_raw_healthy(i).SCbins(j).meanhr_medsteps];
            meanhr_pursteps = [meanhr_pursteps weekly_raw_healthy(i).SCbins(j).meanhr_pursteps];
        end
...
for i = 1:length(weekly_raw_healthy)
    ...
    for j = 1:length(weekly_raw_healthy(i).forecast)
        xdata1p = [xdata1p; strcat('Week', string(weekly_raw_healthy(i).forecast(j).WeekNo),...
            ' Year', string(weekly_raw_healthy(i).forecast(j).YearNo))];
        ydata1p = [ydata1p; weekly_raw_healthy(i).forecast(j).moving_mean_lowsteps];
        ydata1p_sd = [ydata1p_sd; weekly_raw_healthy(i).forecast(j).moving_SD_lowsteps];
    end
...
for i = 1:length(weekly_raw_healthy)
    ...
    figure(i+length(weekly_raw_healthy))
    hold on
    xdata2 = reordercats(categorical(xdata2),xdata2);
    ...
    % Predicted HR values & standard deviation bands
    for j = 1:length(weekly_raw_healthy(i).forecast)
        xdata2p = [xdata2p; strcat('Week', string(weekly_raw_healthy(i).forecast(j).WeekNo),...
            ' Year', string(weekly_raw_healthy(i).forecast(j).YearNo))];
        ydata2p = [ydata2p; weekly_raw_healthy(i).forecast(j).moving_mean_medsteps];
        ydata2p_sd = [ydata2p_sd; weekly_raw_healthy(i).forecast(j).moving_SD_medsteps];
    end
...
for i = 1:length(weekly_raw_healthy)
    ...
    figure(i+2*length(weekly_raw_healthy))
    hold on
    xdata3 = reordercats(categorical(xdata3),xdata3);
    ...
    % Predicted HR values & standard deviation bands
    for j = 1:length(weekly_raw_healthy(i).forecast)
        xdata3p = [xdata3p; strcat('Week', string(weekly_raw_healthy(i).forecast(j).WeekNo),...
            ' Year', string(weekly_raw_healthy(i).forecast(j).YearNo))];
        ydata3p = [ydata3p; weekly_raw_healthy(i).forecast(j).moving_mean_pursteps];
        ydata3p_sd = [ydata3p_sd; weekly_raw_healthy(i).forecast(j).moving_SD_pursteps];
    end
...
% Plot 1st lowest SC category (lowsteps)
for i = 1:length(weekly_raw_healthy)
    if isempty(weekly_raw_healthy(i).forecast)
    else
        xdata1 = []; ydata1 = [];
        xdata1p = []; ydata1p = [];ydata1p_sd = [];
        ...
        lastent = length(weekly_raw_healthy(i).forecast);
        slope = weekly_raw_healthy(i).forecast(lastent).moving_slope_lowsteps;
        int = weekly_raw_healthy(i).forecast(lastent).moving_int_lowsteps;
        y = @(x) (slope*x + int);
...
for i = 1:length(weekly_raw_healthy)
    if isempty(weekly_raw_healthy(i).forecast)
        ...
        lastent = length(weekly_raw_healthy(i).forecast);
        slope = weekly_raw_healthy(i).forecast(lastent).moving_slope_medsteps;
        int = weekly_raw_healthy(i).forecast(lastent).moving_int_medsteps;
        y = @(x) (slope*x + int);
        fplot(y,'-k','LineWidth',2)
        TextLocation(['y = ' num2str(slope),... 
            'x + ' num2str(int)],'Location','northeastoutside');
...
for i = 1:length(weekly_raw_healthy)
    if isempty(weekly_raw_healthy(i).forecast)
        ...
        lastent = length(weekly_raw_healthy(i).forecast);
        slope = weekly_raw_healthy(i).forecast(lastent).moving_slope_pursteps;
        int = weekly_raw_healthy(i).forecast(lastent).moving_int_pursteps;
        y = @(x) (slope*x + int);
        fplot(y,'-k','LineWidth',2)
        TextLocation(['y = ' num2str(slope),... 
            'x + ' num2str(int)],'Location','northeastoutside');
