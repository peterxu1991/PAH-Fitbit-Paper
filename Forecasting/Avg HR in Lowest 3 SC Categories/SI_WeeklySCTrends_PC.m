...
% List indices for PC patients in structure
PC_indices = [4; 6];
...
%% PC Subjects: calculate avg HR for SC categories
...
weekly_raw_PC = this_weekly_raw;


%% Slope / intercept using polyfit for PC Patients
...
%% Plot LSF lines for each individual for PC Patients

figure(1)
hold on
for i = 1:length(PC_indices)
    y = @(x) (weekly_raw_PC(i).meanslope*x + weekly_raw_PC(i).meanint);
    fplot(y,[0,150], 'LineWidth',2)
end
title('LSF Lines for PC Patients (average weekly LSF values)')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f) (PC 1)',weekly_raw_PC(1).meanslope,...
    weekly_raw_PC(1).meanint);
s2 = sprintf('y = (%.2f) x + (%.2f) (PC 9)',weekly_raw_PC(2).meanslope,...
    weekly_raw_PC(2).meanint);
TextLocation([s1,newline,s2],'Location','northwest')
legend('PC 1','PC 9','Location','best')
hold off
...
hold on
for i = 1:length(PC_indices)
    y1 = @(x) (weekly_raw_PC(i).meanslope1*x + weekly_raw_PC(i).meanint1);
    fplot(y1,[0,150], 'LineWidth',2)
end
title('LSF Lines for PC Patients (average weekly LSF values): Region 1')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f) (PC 1)',weekly_raw_PC(1).meanslope1,...
    weekly_raw_PC(1).meanint1);
s2 = sprintf('y = (%.2f) x + (%.2f) (PC 9)',weekly_raw_PC(2).meanslope1,...
    weekly_raw_PC(2).meanint1);
TextLocation([s1,newline,s2],'Location','northwest')
legend('PC 1','PC 9','Location','best')
hold off
...
hold on
for i = 1:length(PC_indices)
    y2 = @(x) (weekly_raw_PC(i).meanslope2*x + weekly_raw_PC(i).meanint2);
    fplot(y2,[0,150], 'LineWidth',2)
end
title('LSF Lines for PC Patients (average weekly LSF values): Region 2')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f) (PC 1)',weekly_raw_PC(1).meanslope2,...
    weekly_raw_PC(1).meanint2);
s2 = sprintf('y = (%.2f) x + (%.2f) (PC 9)',weekly_raw_PC(2).meanslope2,...
    weekly_raw_PC(2).meanint2);
TextLocation([s1,newline,s2],'Location','northwest')
legend('PC 1','PC 9','Location','best')
hold off


%% Plot average HR in different SC bins (weekly) for PC Patients
...
for i = 1:length(weekly_raw_PC)
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata1 = [xdata1; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata1 = [ydata1; weekly_raw_PC(i).SCbins(j).meanhr_lowsteps];  
    end
    figure(i)
    hold on
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata2 = [xdata2; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata2 = [ydata2; weekly_raw_PC(i).SCbins(j).meanhr_medsteps];  
    end
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata3 = [xdata3; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata3 = [ydata3; weekly_raw_PC(i).SCbins(j).meanhr_pursteps];  
    end
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata4 = [xdata4; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata4 = [ydata4; weekly_raw_PC(i).SCbins(j).meanhr_slowwalk];  
    end
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata5 = [xdata5; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata5 = [ydata5; weekly_raw_PC(i).SCbins(j).meanhr_medwalk];  
    end
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata6 = [xdata6; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata6 = [ydata6; weekly_raw_PC(i).SCbins(j).meanhr_briskwalk];  
    end
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata7 = [xdata7; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata7 = [ydata7; weekly_raw_PC(i).SCbins(j).meanhr_fastamb];  
    end
    ...
    for j = 1:length(weekly_raw_PC(i).SCbins)
        xdata8 = [xdata8; strcat('Week', string(weekly_raw_PC(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PC(i).SCbins(j).Year))];
        ydata8 = [ydata8; weekly_raw_PC(i).SCbins(j).meanhr_run];  
    end
...
%% PC Forecasting: calculate mean, SD, and LSF line
...
for i = 1:length(this_weekly_raw)
    if length(this_weekly_raw(i).weekly_raw) < weekthresh
        ...
        numentries = length(daily_Healthy(PC_indices(i)).hr_mu0) - weekthresh;
