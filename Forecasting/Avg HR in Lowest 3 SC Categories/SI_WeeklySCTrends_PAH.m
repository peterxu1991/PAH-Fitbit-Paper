...
% Compile weekly_raw data for each PAH patient.
for i = 1:length(PAH_indices)
    weekly_raw_PAH(i).name = daily_PAH(PAH_indices(i)).name;
    weekly_raw_PAH(i).weekly_raw = struct([]);
    for j = 1:length(daily_PAH(PAH_indices(i)).weekly_raw)
        if isempty(daily_PAH(PAH_indices(i)).weekly_raw(j).YearNo)
        else
            weekly_raw_PAH(i).weekly_raw = [weekly_raw_PAH(i).weekly_raw daily_PAH(PAH_indices(i)).weekly_raw(j)];
        end
    end
end
    ...
    for j = 1:length(this_weekly_raw(i).weekly_raw)
        ...
        for k = 1:length(SCdata)
            ...
            if length(lowsteps) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_lowsteps = [];
                this_weekly_raw(i).SCbins(j).meansc_lowsteps = NaN;
                this_weekly_raw(i).SCbins(j).hr_lowsteps = [];
            ...
            if length(medsteps) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_medsteps = []; 
                this_weekly_raw(i).SCbins(j).meansc_medsteps = NaN;
                this_weekly_raw(i).SCbins(j).hr_medsteps = [];
            ...
            if length(pursteps) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_pursteps = [];
                this_weekly_raw(i).SCbins(j).meansc_pursteps = NaN;
                this_weekly_raw(i).SCbins(j).hr_pursteps = [];
            ...
            if length(slowwalk) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_slowwalk = [];
                this_weekly_raw(i).SCbins(j).meansc_slowwalk = NaN;
                this_weekly_raw(i).SCbins(j).hr_slowwalk = [];
            ...
            if length(medwalk) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_medwalk = [];
                this_weekly_raw(i).SCbins(j).meansc_medwalk = NaN;
                this_weekly_raw(i).SCbins(j).hr_medwalk = [];
            ...
            if length(briskwalk) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_briskwalk = [];
                this_weekly_raw(i).SCbins(j).meansc_briskwalk = NaN;
                this_weekly_raw(i).SCbins(j).hr_briskwalk = [];
            ...
            if length(fastamb) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_fastamb = [];
                this_weekly_raw(i).SCbins(j).meansc_fastamb = NaN;
                this_weekly_raw(i).SCbins(j).hr_fastamb = [];
            ...
            if length(run) > screq - 1
                ...
                this_weekly_raw(i).SCbins(j).sc_run = [];
                this_weekly_raw(i).SCbins(j).meansc_run = NaN;
                this_weekly_raw(i).SCbins(j).hr_run = [];
...
weekly_raw_PAH = this_weekly_raw;


%% Slope / intercept using polyfit for PAH Patients
...
%% Plot LSF lines for each individual for PAH Patients

PAH_all_meanint = struct([]);
PAH_all_meanint(1).meanslope = [];
PAH_all_meanint(1).meanint = [];
PAH_all_meanint(1).meanslope1 = [];
PAH_all_meanint(1).meanint1 = [];
PAH_all_meanint(1).meanslope2 = [];
PAH_all_meanint(1).meanint2 = [];

for i = 1:length(weekly_raw_PAH)
    PAH_all_meanint(1).meanslope = [PAH_all_meanint(1).meanslope weekly_raw_PAH(i).meanslope];
    PAH_all_meanint(1).meanint = [PAH_all_meanint(1).meanint weekly_raw_PAH(i).meanint];
    PAH_all_meanint(1).meanslope1 = [PAH_all_meanint(1).meanslope1 weekly_raw_PAH(i).meanslope1];
    PAH_all_meanint(1).meanint1 = [PAH_all_meanint(1).meanint1 weekly_raw_PAH(i).meanint1];
    PAH_all_meanint(1).meanslope2 = [PAH_all_meanint(1).meanslope2 weekly_raw_PAH(i).meanslope2];
    PAH_all_meanint(1).meanint2 = [PAH_all_meanint(1).meanint2 weekly_raw_PAH(i).meanint2];
end

for k = 1:length(PAH_all_meanint(1).meanslope2)
    nanind = find(isnan(PAH_all_meanint(1).meanslope2));
    if isempty(nanind)
        continue
    else
        for l = 1:length(nanind)
            PAH_all_meanint(1).meanslope2(nanind(length(nanind)-l+1)) = [];
            PAH_all_meanint(1).meanint2(nanind(length(nanind)-l+1)) = [];
        end
    end
end
    
PAH_all_meanint(1).allmeanslope = mean(PAH_all_meanint(1).meanslope);
PAH_all_meanint(1).allmeanint = mean(PAH_all_meanint(1).meanint);
PAH_all_meanint(1).allmeanslope1 = mean(PAH_all_meanint(1).meanslope1);
PAH_all_meanint(1).allmeanint1 = mean(PAH_all_meanint(1).meanint1);
PAH_all_meanint(1).allmeanslope2 = mean(PAH_all_meanint(1).meanslope2);
PAH_all_meanint(1).allmeanint2 = mean(PAH_all_meanint(1).meanint2);

figure(1)
hold on
for i = 1:length(PAH_indices)
    y = @(x) (weekly_raw_PAH(i).meanslope*x + weekly_raw_PAH(i).meanint);
    fplot(y,[0,150], 'LineWidth',2)
end
title('LSF Lines for PAH Patients (average weekly LSF values)')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f)',PAH_all_meanint(1).allmeanslope,...
    PAH_all_meanint(1).allmeanint);
TextLocation(s1,'Location','best')
hold off
...
hold on
for i = 1:length(PAH_indices)
    y1 = @(x) (weekly_raw_PAH(i).meanslope1*x + weekly_raw_PAH(i).meanint1);
    fplot(y1,[0,150], 'LineWidth',2)
end
title('LSF Lines for PAH Patients (average weekly LSF values): Region 1')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f)',PAH_all_meanint(1).allmeanslope1,...
    PAH_all_meanint(1).allmeanint1);
TextLocation(s1,'Location','best')
hold off
...
hold on
for i = 1:length(PAH_indices)
    y2 = @(x) (weekly_raw_PAH(i).meanslope2*x + weekly_raw_PAH(i).meanint2);
    fplot(y2,[0,150], 'LineWidth',2)
end
title('LSF Lines for PAH Patients (average weekly LSF values): Region 2')
xlabel('SPM')
ylabel('Mean HR (BPM)')
s1 = sprintf('y = (%.2f) x + (%.2f)',PAH_all_meanint(1).allmeanslope2,...
    PAH_all_meanint(1).allmeanint2);
TextLocation(s1,'Location','best')
hold off


%% Plot average HR in different SC bins (weekly) for PAH Patients

% Which patient are you interested in generating graphs for?
pah_num = 7;

% Average HR
for i = pah_num
    % 1:length(weekly_raw_PAH)
    xdata1 = []; ydata1 = [];
    xdata2 = []; ydata2 = [];
    xdata3 = []; ydata3 = [];
    xdata4 = []; ydata4 = [];
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata1 = [xdata1; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata1 = [ydata1; weekly_raw_PAH(i).SCbins(j).meanhr_lowsteps];  
    end
    figure(i)
    hold on
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata2 = [xdata2; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata2 = [ydata2; weekly_raw_PAH(i).SCbins(j).meanhr_medsteps];  
    end
    xdata2 = reordercats(categorical(xdata2),xdata2);
    plot(xdata2,ydata2,'Color',[0.3010, 0.7450, 0.9330],'LineWidth',1)
    
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata3 = [xdata3; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata3 = [ydata3; weekly_raw_PAH(i).SCbins(j).meanhr_pursteps];  
    end
    xdata3 = reordercats(categorical(xdata3),xdata3);
    plot(xdata3,ydata3,'Color','m','LineWidth',1)
    
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata4 = [xdata4; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata4 = [ydata4; weekly_raw_PAH(i).SCbins(j).meanhr_slowwalk];  
    end
    xdata4 = reordercats(categorical(xdata4),xdata4);
    plot(xdata4,ydata4,'Color','c','LineWidth',1)
    
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata5 = [xdata5; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata5 = [ydata5; weekly_raw_PAH(i).SCbins(j).meanhr_medwalk];  
    end
    xdata5 = reordercats(categorical(xdata5),xdata5);
    plot(xdata5,ydata5,'Color','r','LineWidth',1)
    
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata6 = [xdata6; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata6 = [ydata6; weekly_raw_PAH(i).SCbins(j).meanhr_briskwalk];  
    end
    xdata6 = reordercats(categorical(xdata6),xdata6);
    plot(xdata6,ydata6,'Color','k','LineWidth',1)
    
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata7 = [xdata7; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata7 = [ydata7; weekly_raw_PAH(i).SCbins(j).meanhr_fastamb];  
    end
    xdata7 = reordercats(categorical(xdata7),xdata7);
    plot(xdata7,ydata7,'Color','g','LineWidth',1)
    
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata8 = [xdata8; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        ydata8 = [ydata8; weekly_raw_PAH(i).SCbins(j).meanhr_run];  
    end
    ...
    title(sprintf('Mean HR for PAH Subject %s',string(PAH_indices(i))))
    xlabel('Time')
    ylabel('Mean HR (BPM)')
    hold off
end
...
for i = pah_num
    ...
    exind = [];
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        if isempty(weekly_raw_PAH(i).SCbins(j).Week)
            exind = [exind, j];
        else
            continue
        end
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        xdata1 = [xdata1; strcat('Week', string(weekly_raw_PAH(i).SCbins(j).Week),...
            ' Year', string(weekly_raw_PAH(i).SCbins(j).Year))];
        % Total # of minutes (of wear time)
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_pursteps) + length(weekly_raw_PAH(i).SCbins(j).sc_slowwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata1 = [ydata1; length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) / totalmin * 100];
    end
    figure(i + length(PAH_indices))
    hold on
    xdata1 = reordercats(categorical(xdata1),xdata1);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata2 = [ydata2; length(weekly_raw_PAH(i).SCbins(j).sc_medsteps) / totalmin * 100];  
    end
    xdata2 = reordercats(categorical(xdata2),xdata2);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata3 = [ydata3; length(weekly_raw_PAH(i).SCbins(j).sc_pursteps) / totalmin * 100];  
    end
    xdata3 = reordercats(categorical(xdata3),xdata3);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata4 = [ydata4; length(weekly_raw_PAH(i).SCbins(j).sc_slowwalk) / totalmin * 100];  
    end
    xdata4 = reordercats(categorical(xdata4),xdata4);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata5 = [ydata5; length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) / totalmin * 100];  
    end
    xdata5 = reordercats(categorical(xdata5),xdata5);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata6 = [ydata6; length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk) / totalmin * 100];  
    end
    xdata6 = reordercats(categorical(xdata6),xdata6);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata7 = [ydata7; length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) / totalmin * 100];  
    end
    xdata7 = reordercats(categorical(xdata7),xdata7);
    if isempty(exind)
        continue
    ...
    for j = 1:length(weekly_raw_PAH(i).SCbins)
        ...
        totalmin = length(weekly_raw_PAH(i).SCbins(j).sc_lowsteps) + length(weekly_raw_PAH(i).SCbins(j).sc_medsteps)...
            ...
            + length(weekly_raw_PAH(i).SCbins(j).sc_medwalk) + length(weekly_raw_PAH(i).SCbins(j).sc_briskwalk)...
            + length(weekly_raw_PAH(i).SCbins(j).sc_fastamb) + length(weekly_raw_PAH(i).SCbins(j).sc_run);
        ydata8 = [ydata8; length(weekly_raw_PAH(i).SCbins(j).sc_run) / totalmin * 100];  
    end
    xdata8 = reordercats(categorical(xdata8),xdata8);
    if isempty(exind)
        continue
    ...
    legend('Low Steps', 'Medium Steps', 'Purposeful Steps', 'Slow Walk', 'Medium Walk', ...
        'Brisk Walk', 'Fast Ambulation', 'Run','Location','eastoutside')
    title(sprintf('Percentage of Minutes in Each SC Category for PAH Subject %s',string(PAH_indices(i))))
    xlabel('Time')
...
%% PAH Forecasting: calculate mean, SD, and LSF line
...
for i = pah_num
    if length(this_weekly_raw(i).weekly_raw) < weekthresh
        ...
        numentries = length(daily_Healthy(PAH_indices(i)).hr_mu0) - weekthresh;
        ...
        for j = 1:numentries
            ...
            this_weekly_raw(i).forecast(j).moving_mean_lowsteps = mean(meanhr_lowsteps(1:(weekthresh+j-1)));
            this_weekly_raw(i).forecast(j).moving_SD_lowsteps = std(meanhr_lowsteps(1:(weekthresh+j-1)));
            this_weekly_raw(i).forecast(j).moving_mean_medsteps = mean(meanhr_medsteps(1:(weekthresh+j-1)));
            this_weekly_raw(i).forecast(j).moving_SD_medsteps = std(meanhr_medsteps(1:(weekthresh+j-1)));
            this_weekly_raw(i).forecast(j).moving_mean_pursteps = mean(meanhr_pursteps(1:(weekthresh+j-1)));
            this_weekly_raw(i).forecast(j).moving_SD_pursteps = std(meanhr_pursteps(1:(weekthresh+j-1)));
        end
...
%% Create .mat file to store 6 mean/intercept parameters

all_meanint = struct([]);

for i = 1:length(weekly_raw_healthy)
    all_meanint(i).name = weekly_raw_healthy(i).name;
    all_meanint(i).weekly_slope = weekly_raw_healthy(i).weekly_slope;
    all_meanint(i).weekly_int = weekly_raw_healthy(i).weekly_int;
    all_meanint(i).weekly_slope1 = weekly_raw_healthy(i).weekly_slope1;
    all_meanint(i).weekly_int1 = weekly_raw_healthy(i).weekly_int1;
    all_meanint(i).weekly_slope2 = weekly_raw_healthy(i).weekly_slope2;
    all_meanint(i).weekly_int2 = weekly_raw_healthy(i).weekly_int2;
end

for i = 1:length(weekly_raw_PC)
    all_meanint(i + length(weekly_raw_healthy)).name = weekly_raw_PC(i).name;
    all_meanint(i + length(weekly_raw_healthy)).weekly_slope = weekly_raw_PC(i).weekly_slope;
    all_meanint(i + length(weekly_raw_healthy)).weekly_int = weekly_raw_PC(i).weekly_int;
    all_meanint(i + length(weekly_raw_healthy)).weekly_slope1 = weekly_raw_PC(i).weekly_slope1;
    all_meanint(i + length(weekly_raw_healthy)).weekly_int1 = weekly_raw_PC(i).weekly_int1;
    all_meanint(i + length(weekly_raw_healthy)).weekly_slope2 = weekly_raw_PC(i).weekly_slope2;
    all_meanint(i + length(weekly_raw_healthy)).weekly_int2 = weekly_raw_PC(i).weekly_int2;
end

for i = 1:length(weekly_raw_COPD)
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).name = ...
        weekly_raw_COPD(i).name;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).weekly_slope = ...
        weekly_raw_COPD(i).weekly_slope;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).weekly_int = ...
        weekly_raw_COPD(i).weekly_int;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).weekly_slope1 = ...
        weekly_raw_COPD(i).weekly_slope1;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).weekly_int1 = ...
        weekly_raw_COPD(i).weekly_int1;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).weekly_slope2 = ...
        weekly_raw_COPD(i).weekly_slope2;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC)).weekly_int2 = ...
        weekly_raw_COPD(i).weekly_int2;
end

for i = 1:length(weekly_raw_PAH)
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).name = weekly_raw_PAH(i).name;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).weekly_slope = weekly_raw_PAH(i).weekly_slope;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).weekly_int = weekly_raw_PAH(i).weekly_int;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).weekly_slope1 = weekly_raw_PAH(i).weekly_slope1;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).weekly_int1 = weekly_raw_PAH(i).weekly_int1;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).weekly_slope2 = weekly_raw_PAH(i).weekly_slope2;
    all_meanint(i + length(weekly_raw_healthy) + length(weekly_raw_PC) + ...
        length(weekly_raw_COPD)).weekly_int2 = weekly_raw_PAH(i).weekly_int2;
end
